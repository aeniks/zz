...
####
printf %b "done\n"; for i in $(cat ${igfile}.log|nl --number-width=1|cut -c1); do printf -v igf %b "$(sed -n ${i}p ${igfile}.log)"; printf -v igfw %b "\n$(printf %b "$igf"|cut -f1 -d" ")"; (figlet -w ${COLUMNS-222} -f Roman "$(printf %b "\n\n$igf"|cut -f1 -d" ")"|tr "''" "+"|bat -ppfljava; printf %b "\n\e[2A\e[2m${igf}\e[0m\n"|cut -f2 -d" "|bat -ppflr; printf %b "\n${igf}"|cut -f3- -d" "|fmt -g 55|gum style --border none  --align center --padding "0" --margin "0" | gum style --margin "1 40 22 40" --padding "1 2" --border normal | bat -ppfljava) | aha | tail -n+11 | head -n-3 | sed -e 's/style\=//g' -e s/\"color\:/color\=\'/g -e s/[\"]/\'/g -e s/\;\'\>/\'\>/g|col -xb|ul > ${igfw}.html; convert -background "#222222" -gravity center pango:"<tt>$(cat $igfw.html)</tt>" ${igfw}.jpg; done;
####
####
